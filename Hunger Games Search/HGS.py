...
def main(Dim):
    ...
    for suite in CEC_dict.keys():
        ...
        for i in range(len(CEC_dict[SuiteName])):
            ...
            np.savetxt("./HGS_Data/" + SuiteName + "/F" + str(i+1) + "_" + str(Dim) + "D.csv", All_Trial_Best, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('./HGS_Data/CEC2013') == False:
        os.makedirs('./HGS_Data/CEC2013')
    if os.path.exists('./HGS_Data/CEC2014') == False:
        os.makedirs('./HGS_Data/CEC2014')
    Dims = [10, 30, 50]
    for Dim in Dims:
        main(Dim)