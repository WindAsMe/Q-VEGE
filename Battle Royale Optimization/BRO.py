    ...
    for suite in CEC_dict.keys():
        ...
        for i in range(len(CEC_dict[SuiteName])):
            ...
            for j in range(trials):
                bro_solver = BRO.OriginalBRO(epoch=MaxIter, pop_size=PopSize)
                np.random.seed(2022 + 88 * j)
                best_position, best_fitness_value = bro_solver.solve(problem_dict)
                All_Trial_Best.append(bro_solver.history.list_global_best_fit)
            np.savetxt("./BRO_Data/" + SuiteName + "/F" + str(i+1) + "_" + str(Dim) + "D.csv", All_Trial_Best, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('./BRO_Data/CEC2013') == False:
        os.makedirs('./BRO_Data/CEC2013')
    if os.path.exists('./BRO_Data/CEC2014') == False:
        os.makedirs('./BRO_Data/CEC2014')
    Dims = [10, 30, 50]
    for Dim in Dims:
        main(Dim)