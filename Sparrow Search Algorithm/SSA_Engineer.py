...
def main():
    ...
    for i in range(len(Dims)-1):
        ...
        for j in range(trials):
            ssa_solver = SSA.OriginalSSA(epoch=MaxIter, pop_size=PopSize, max_age=Dims[i])
        ...
        np.savetxt("./SSA_Data/" + SuiteName + "/" + problem_names[i] + ".csv", All_Trial_Best, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('./SSA_Data/Engineer') == False:
        os.makedirs('./SSA_Data/Engineer')
    main()