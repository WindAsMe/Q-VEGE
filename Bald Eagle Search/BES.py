    ...
    for suite in CEC_dict.keys():
        ...
        for i in [11]:
            ...
            for j in range(trials):
                bes_solver = BES.OriginalBES(epoch=MaxIter, pop_size=PopSize)
                np.random.seed(2022 + 88 * j)
                best_position, best_fitness_value = bes_solver.solve(problem_dict)
                All_Trial_Best.append(bes_solver.history.list_global_best_fit)
            np.savetxt("./BES_Data/" + SuiteName + "/F" + str(i+1) + "_" + str(Dim) + "D.csv", All_Trial_Best, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('./BES_Data/CEC2013') == False:
        os.makedirs('./BES_Data/CEC2013')
    if os.path.exists('./BES_Data/CEC2014') == False:
        os.makedirs('./BES_Data/CEC2014')
    Dims = [50]
    for Dim in Dims:
        main(Dim)