    ...
    for suite in CEC_dict.keys():
        ...
        for i in range(len(CEC_dict[SuiteName])):
            ...
            for j in range(trials):
                de_solver = DE.BaseDE(epoch=MaxIter, pop_size=PopSize)
                np.random.seed(2022 + 88 * j)
                best_position, best_fitness_value = de_solver.solve(problem_dict)
                All_Trial_Best.append(de_solver.history.list_global_best_fit)
            np.savetxt("./DE_Data/" + SuiteName + "/F" + str(i+1) + "_" + str(Dim) + "D.csv", All_Trial_Best, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('./DE_Data/CEC2013') == False:
        os.makedirs('./DE_Data/CEC2013')
    if os.path.exists('./DE_Data/CEC2014') == False:
        os.makedirs('./DE_Data/CEC2014')
    Dims = [10, 30, 50]
    for Dim in Dims:
        main(Dim)